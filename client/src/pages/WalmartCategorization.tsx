...
type SplitItem = {
  id: string; verticalId: string; verticalName: string; category: string | null;
  customCategory: string | null; propertyName: string | null;
  ...
  return (
    <div className="flex items-center gap-2 p-2.5 bg-muted rounded-lg flex-wrap">
      <span className="text-sm flex-1 min-w-0">
        <span className="font-medium">{split.verticalName}</span>: {split.category || split.customCategory || "No category"}
...
export default function WalmartCategorization() {
  ...
  const { data: config } = trpc.walmartCategorization.getConfig.useQuery();
  const { data: stats, refetch: refetchStats } = trpc.walmartCategorization.getStats.useQuery();
  const { data: ordersData, refetch: refetchOrders } = trpc.walmartCategorization.getOrders.useQuery({
    status: statusFilter, page, pageSize: 20,
  });

  // Fetch saved categorization for the selected order
  const { data: savedCategorization } = trpc.walmartCategorization.getCategorization.useQuery(
  ...
  const categorizeMutation = trpc.walmartCategorization.categorize.useMutation({
    onSuccess: () => {
  ...
  const splitMutation = trpc.walmartCategorization.categorizeSplit.useMutation({
    onSuccess: () => {
  ...
  const skipMutation = trpc.walmartCategorization.skip.useMutation({
    onSuccess: () => {
  ...
  const memoMutation = trpc.walmartCategorization.updateMemo.useMutation({
    onSuccess: () => {
  ...
  const availableCategories = useMemo(() => {
    if (!currentVerticalConfig || !config) return [];
    return (config.categories as any)[currentVerticalConfig.categoryKey] || [];
  }, [currentVerticalConfig, config]);
  ...
  function handleCategorize() {
    if (!currentOrder || !selectedVertical || !currentVerticalConfig) return;
  ...
  function handleSplitSave() {
    ...
    splitMutation.mutate({ orderId: currentOrder.id, splits });
  }
  ...
  function getWalmartOrderUrl(order: any) {
    if (order.walmartUrl) return order.walmartUrl;
    if (order.walmartOrderId) {
      const base = `https://www.walmart.com/orders/${order.walmartOrderId}`;
    ...
    // Fallback: always show a link to Walmart order history so user can find the order
                ...
                <div key={idx} className="flex items-center gap-3 p-2.5 rounded-lg bg-background border">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">
                      {cat.category || cat.customCategory || "No category"}
                      {cat.propertyName && ` • ${cat.propertyName}`}
                    </p>
                  </div>
                  {isSplit && (
            ...
            <CardContent className="p-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {order.location && (
                    <p className="text-xs text-muted-foreground truncate mt-0.5">{order.location}</p>
                  )}
                  {order.memo && (
  ...
  const detailPanelContent = !currentOrder ? (
    ...
    const walmartUrl = getWalmartOrderUrl(currentOrder);
    ...
    return (
      <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              <div className="flex items-start gap-3 sm:gap-4">
                ...
                <div>
                  ...
                  {currentOrder.seller && (
                    <p className="text-sm text-muted-foreground mt-1">
                      <span className="font-medium">Seller:</span> {currentOrder.seller}
                    </p>
                  )}
              ...
              <div className="flex flex-row sm:flex-col gap-2 flex-shrink-0">
                {walmartUrl && (
                  <Button variant="default" size="sm" asChild className="gap-2 flex-1 sm:flex-initial">
                    <a href={walmartUrl} target="_blank" rel="noopener noreferrer">
                      <ExternalLink className="w-4 h-4" />
                      <span className="hidden sm:inline">View on Walmart</span>
                      <span className="sm:hidden">Walmart</span>
                    </a>
                  </Button>
                )}
        ...
        {/* Order Items (embedded Walmart content) */}
        {orderItemsContent}
          ...
          <Button
            ...
            onClick={() => {
              ...
              createExtractionMutation.mutate({
                orderId: currentOrder.id,
                orderType: "walmart_order",
                filename: `invoice-${currentOrder.walmartOrderId || currentOrder.id}.pdf`,
              });
              // Post message to extension bridge to open Walmart and capture
              if (walmartUrl && walmartUrl !== "https://www.walmart.com/account/orders") {
                window.postMessage({
                  type: "GEEVES_CAPTURE_REQUEST",
                  payload: { orderId: currentOrder.id, walmartUrl },
                }, "*");
              } else if (walmartUrl) {
                // Open Walmart order history so user can navigate to the order
          ...
          <CardContent className="p-4 md:p-5">
            ...
            {currentOrder.receiptUrl ? (
              ...
              <div className="border rounded-lg p-6 text-center bg-muted/20">
                ...
                <p className="text-xs text-muted-foreground mt-1">Upload a receipt or use the Chrome Extension to capture one from Walmart</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        {showCategorizationForm && (
          <Card>
            <CardContent className="p-4 md:p-5">
              ...
              {selectedVertical && (
                <div className="mb-4">
                  <label className="text-sm font-medium mb-2 block">QBO Category</label>
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-6 max-w-[1600px] mx-auto">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-foreground">Walmart Order Categorization</h1>
        <p className="text-sm text-muted-foreground mt-1">Assign orders to verticals and QBO categories for reconciliation</p>
        ...
        <div className="mb-4 p-3 rounded-lg border border-blue-500/30 bg-blue-500/5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-blue-600">
              Install the Geeves Invoice Capture extension to auto-extract line items from Walmart invoices.
            </span>
          </div>
      ...
      {/* Filter tabs */}
      <Tabs value={statusFilter} onValueChange={(v) => { setStatusFilter(v as any); setPage(1); setSelectedOrderId(null); setMobileView("list"); }} className="mb-4">
