...
async function main() {
  ...
  const trulyMissing = ['HMMXW8WFK4','3890454','5914779093','6108345671','6152048756','6186367098','6199754500','6259614150','6300927700','6320697500','6352150000'];
  
  const meta = await sheets.spreadsheets.get({ spreadsheetId: SPREADSHEET_ID });
  const sheetNames = meta.data.sheets?.map(s => s.properties?.title || '') || [];
  const enhancedTabs = sheetNames.filter(n => n.toLowerCase().includes('enhanced'));
  
  console.log('=== 11 BOOKINGS TRULY MISSING FROM DATABASE ===\n');
  ...
  for (const tab of enhancedTabs) {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (conf && trulyMissing.includes(conf)) {
        const fields: string[] = [];
