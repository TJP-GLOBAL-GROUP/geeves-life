...
**Do not open the sprint on v2.1.** Spend the first session producing v2.2, which differs from v2.1 in six specific ways: it adds a production MySQL migration alongside the SQLite staging pack and defines the transfer step between them, resolves the chart-of-accounts divergence, settles the vertical identity model by bridging codes to UUIDs, settles the two-household question, adopts the access design in Part 3 including the correction to the G6 role matrix, and records Square as an explicit non-goal. Every one of those is a decision rather than an implementation, and none of them takes long — but each of them, left open, invalidates work done downstream.
