...
## ✅ COMPLETE — Edits API (Reconciliation Explorer write-back)

- [x] edit_log table created in live TiDB database
- [x] edit_log added to drizzle/schema.ts
- [x] GET /api/health — returns {ok:true, db:"up"}, 503 on failure
- [x] POST /api/edits — handles edits, splits, field_edits; all-or-nothing; EDIT_SECRET auth; audit log
- [x] GET /api/edit-log — returns recent audit rows with limit param
- [x] CORS enabled for all origins on all three routes
- [x] EDIT_SECRET env var configured via webdev_request_secrets
- [x] createEditsApiRouter registered in server/_core/index.ts at /api
- [x] 7 vitest tests passing (health, auth, validation, edits, edit-log, CORS preflight)
- [x] Production build clean (28s, zero errors, zero TypeScript errors)
...
- [x] Add GCS_BUCKET_NAME secret
- [x] Add GOOGLE_APPLICATION_CREDENTIALS_JSON secret (service account with Storage Object Admin)
- [x] Install @google-cloud/storage npm package
- [x] Add GCS env vars to server/_core/env.ts
- [x] Replace server/storage.ts Manus Forge implementation with GCS implementation
- [x] Write vitest tests for GCS storagePut / storageGet (8/8 passing)
- [x] Verify pnpm build passes with zero errors (28s, zero errors)
- [x] Save checkpoint and guide user to publish (checkpoint 7232ccbd)

## Gap Fill (GAP_FILL_PROMPT.docx — 2026-07-25)

### Section A: CRITICAL
- [x] A1: Fix vertical color bug in expenseCategorisation.ts — replace hardcoded VERTICAL_STYLES map with DB field access (v.icon, v.color)
- [x] A2: Add invoice bridge endpoints to invoiceExtraction.ts — list query and convertToExpense mutation
- [x] A3: Run schema migrations — platform_fee_configurations table, vertical_expense_configs table, expenses assignable columns, invoice_extractions status columns, seed platform presets

### Section B: HIGH — Shadow Block Hardening
- [x] B1: P-1 loud verticalId error in eventPropagation.ts — change console.log to console.error + throw so retry queue triggers
- [x] B2: P-3 batch calendar access in eventPropagation.ts + db.ts — add getAllMemberCalendarAccessForHousehold, replace per-call shouldExcludeForMember
- [x] B3: P-4b DST-safe end date in eventPropagation.ts — replace +86400000 with setUTCDate(+1)
- [x] B4: Webhook token validation in calendarWebhook.ts — add ENABLE_WEBHOOK_TOKEN_CHECK guard + webhook_tokens table

### Section C: MEDIUM — New Features
- [x] C1: Create server/services/platformReconciliation.ts — reconcileBooking and reconcilePropertyBookings
- [x] C2: Create server/routers/platformConfig.ts — CRUD router for platform_fee_configurations
- [x] C3: Create server/routers/onboarding.ts — 6-step onboarding router with autoApplyPlatformPreset
- [x] C3b: Wire onboardingRouter and platformConfigRouter into routers.ts appRouter

### Section D: LOW
- [x] D1: Softer cap landing in eventPropagation.ts — queue remaining blocks on hourly cap instead of throwing
- [x] D2: Better verticalId logging — include calendar name and householdId in error message
