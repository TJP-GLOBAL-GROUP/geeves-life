...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  try {
    ...
    console.error("[Database] Failed to upsert user:", error); throw error;
  }
}
...
export async function getShoppingListItemById(id: number, userId: number) {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select().from(shoppingListItems).where(and(eq(shoppingListItems.id, id), eq(shoppingListItems.userId, userId))).limit(1);
  return result[0];
}
...
export async function deleteShoppingListItem(id: number, userId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getSpendingByCategory(userId: number) {
  ...
  return db.select({
    ...
    .from(transactions)
    .where(and(eq(transactions.userId, userId), eq(transactions.type, "expense"))).groupBy(transactions.expenseCategory);
}
...
export async function getLatestExchangeRate(fromCurrency: string, toCurrency: string) {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select().from(exchangeRates).where(and(eq(exchangeRates.fromCurrency, fromCurrency), eq(exchangeRates.toCurrency, toCurrency))).orderBy(desc(exchangeRates.fetchedAt)).limit(1);
  return result[0];
}
    ...
    for (const item of items) {
      ...
      for (const keyword of keywords) {
        ...
        if (itemNameLower === kwLower) score = 100;
          ...
          const itemWords = itemNameLower.split(/\s+/).filter(Boolean);
...
export async function updateShoppingSession(id: number, userId: number, data: Partial<InsertShoppingSession>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(shoppingSessions).set(data).where(eq(shoppingSessions.id, id));
}

export async function deleteShoppingSession(id: number, userId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.delete(shoppingSessions).where(eq(shoppingSessions.id, id));
}
...
export async function getEvents(householdId: string, startTime: number, endTime: number) {
  ...
  // Aug 8 2026 (fix/recurring-delete-resurrection): exclude cancelled tombstones.
  // Incremental sync previously upserted Google 'cancelled' tombstones as rows;
  // without this filter, deleted events (especially recurring series deleted with
  // scope=all/following) resurrected on the calendar.
  const { lt, gt, ne } = await import("drizzle-orm");
...
/** Look up a single event by its Google externalId within a calendar. */
export async function getEventByExternalId(calendarId: string, externalId: string) {
  const db = await getDb();
  if (!db) return null;
  const rows = await db.select().from(events).where(
    and(eq(events.calendarId, calendarId), eq(events.externalId, externalId))
  ).limit(1);
  return rows[0] ?? null;
}
...
export async function upsertPropertyPrepRule(data: InsertPropertyPrepRule) {
  ...
  if (existing) {
    await db.update(propertyPrepRules).set({ ...data, updatedAt: new Date() }).where(eq(propertyPrepRules.id, existing.id));
  } else {
...
export async function isMemberRestrictedFromDataCategory(opts: {
  ...
  const hiddenRoles = (policy.hiddenRoles ?? []) as string[];
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        inArray(verticalIds, verticalIds)
      )
    );
  for (const row of rows) {
...
export async function getPropertyRevenueSummaryWithTax(propertyId: string, fromTs?: number, toTs?: number) {
  ...
  return {
    ...
    taxOwedByHost: Math.round(taxOwedByHost * 100) / 100,
